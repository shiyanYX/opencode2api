...
- **配额熔断（quota halt）**：节点池无可用节点（出口回退直连）且直连出口被上游 429 限流时进入的进程级熔断（`quota_halt.go`）。触发即在同一次请求内跳出重试循环、把上游 429 原始 body 原样返回下游，并置全局标记；熔断期间**免费层请求在 `callOpenCodeAPI`/`callOpenCodeAPIStream` 入口直接返回 429**（重放保存的错误 body，零上游调用、零节点切换）。恢复是**惰性 + 事件驱动**：每个请求检查 `nodePool.hasEligibleNode()`，一旦出现 `available` 节点（含配额冷却到期被 `sweepExpiredLocked` 翻回的）立即解除，无定时器、不持久化、不跨重启。仅免费层（`TierFree`，即 `路由: public`）受影响；付费层的 `insufficient_quota`/`credits_error` 是账号计费问题，与出口 IP 无关，不参与熔断。注意与「配额切换预算」区分：预算管单请求内切几个节点，熔断管「所有节点都没了且直连也被限流」时整段停止转发。
- **上游客户端形态（upstream client shape）**：OpenCode Zen 免费层的鉴权依据——上游按「请求像不像官方 opencode 客户端」放行，而非按账号或出口 IP。截至 2026-09-21 实测**五道条件**：`x-opencode-session` 的官方 ID 形态（`ses_` + 12 位小写十六进制 + 14 位 base62）、`User-Agent` 的 `opencode/<semver>` 形态且**版本 >= 1.18.0**、UA 必须存在且形如 `opencode/<semver>`、上游 `stream:true`、上游 `tools` 含必需官方工具名。**两种失配错误要分清**：形态问题（session/stream/tools/UA 缺失或非 semver）返回 `FreeTierError`（403）；UA 格式正确但版本过低返回 `UpgradeRequired`（**426**）。二者都与按出口 IP 限流的 `FreeUsageLimitError`（429）不同——前两者只能靠形态对齐解决，换节点/换 key 均无效。完整矩阵与重新定位流程见 `docs/UPSTREAM-COMPAT.md`，决策见 `docs/adr/0002-zen-free-tier-client-fingerprint.md`。
- **UA 版本下限与兜底（OC version floor & fallback）**：上游要求 UA 里的 opencode 版本 >= `minFreeTierOCVersion`（1.18.0）。版本号取自 npm `opencode-ai` latest，探测失败时回退——**兜底值必须自身满足门槛**，否则一次网络抖动就会让全量请求变 426（历史事故：回退常量 `1.15.3`）。现由 `oc_version.go` 统一处理：`lastGoodOCVersion()` 优先返回最近一次成功探测到的版本，低于门槛的版本不入缓存，`fallbackOCVersion`（1.18.31）满足门槛并有单测断言；收到 426 时刷新一次版本并重试，可自愈。
