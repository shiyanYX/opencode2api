...
- 概览·模型能力显示全部上游模型：`/api/models`（管理面板模型列表）从纯 ID 字符串改为完整模型对象，并填充 models.dev 目录的上下文窗口/最大输出/输入模态；概览「模型能力」表格改用该接口，显示全部上游模型（含 `-free` 免费模型，带「免费」徽标并高亮），与「模型映射」自动行数量一致。
